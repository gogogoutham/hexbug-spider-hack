// Establish firebase URL specific to agent - firebase and agent ID should be adjusted to your configuration

//Interface to Firebase Streaming API taken from here - https://github.com/beardedinventor/ElectricImp-FirebaseIO
const FIREBASE_URL = "https://[[my-firebase]].firebaseio-demo.com/[[my-agent-id]]/"
const FIREBASE_AUTH = ""
...
//Register event handlers for firebase updates
firebase.on("/", function(data) {
    if ("moveForward" in data) {
        server.log("Agent received moveForward state " + data.moveForward + " from firebase. Sending to device.");
        device.send("moveForward", data.moveForward.tointeger());
    }
    
    if ("moveBackward" in data) {
        server.log("Agent received moveBackward state " + data.moveBackward + " from firebase. Sending to device.");
        device.send("moveBackward", data.moveBackward.tointeger());
    }
    
    if ("turnLeft" in data) {
        server.log("Agent received turnLeft state " + data.turnLeft + " from firebase. Sending to device.");
        device.send("turnLeft", data.turnLeft.tointeger());
    }
    
    if ("turnRight" in data) {
        server.log("Agent received turnRight state " + data.turnRight + " from firebase. Sending to device.");
        device.send("turnRight", data.turnRight.tointeger());
    }
    
});

//Register alternate event handlers for firebase updates
firebase.on("/moveForward", function(state) {
    server.log("Agent received moveForward state " + state + " from firebase. Sending to device.");
    device.send("moveForward", state);
});

firebase.on("/moveBackward", function(state) {
    server.log("Agent received moveBackward state " + state + " from firebase. Sending to device.");
    device.send("moveBackward", state);
});

firebase.on("/turnLeft", function(state) {
    server.log("Agent received turnLeft state " + state + " from firebase. Sending to device.");
    device.send("turnLeft", state);
});

firebase.on("/turnRight", function(state) {
    server.log("Agent received turnRight state " + state + " from firebase. Sending to device.");
    device.send("turnRight", state);
});
